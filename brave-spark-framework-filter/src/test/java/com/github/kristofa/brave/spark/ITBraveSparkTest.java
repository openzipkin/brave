...
public class ITBraveSparkTest extends ITHttpServer {
    ...
    protected void init(Brave brave, SpanNameProvider spanNameProvider) throws Exception {
        stop();
        Spark.before(BraveSparkFilter.builder(brave).spanNameProvider(spanNameProvider).build().requestFilter());
        ...
        Spark.afterAfter(BraveSparkFilter.builder(brave).spanNameProvider(spanNameProvider).build().responseFilter());
