...
public class TracingHttpClientBuilderTest {
  TracingHttpClientBuilder filter =
      new TracingHttpClientBuilder(HttpTracing.create(Tracing.newBuilder().build()));
  @Mock Span span;
  ...
  @Test public void parseServerAddress_doesntSkipWhenServiceNameIsNotBlank() {
    filter = new TracingHttpClientBuilder(
