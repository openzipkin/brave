...
public class TraceContextBinaryMarshallerBenchmarks {
  ...
  static final byte[] serialized = TraceContextBinaryMarshaller.toBytes(context);

  @Benchmark public byte[] toBytes() {
    return TraceContextBinaryMarshaller.toBytes(context);
  }

  @Benchmark public void extract() {
    TraceContextBinaryMarshaller.extract(serialized, new MutableTraceContext());
  }
