...
public class CurrentTraceContextAssemblyTrackingMatrixTest {
  ...
  @Test public void scalarCallable_completable_assembleInScope_subscribeNoScope() {
    Completable source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableCompletable(null)
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableCompletable(null, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_completable_assembleInScope_subscribeInScope() {
    Completable source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableCompletable(subscribeContext)
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableCompletable(subscribeContext, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_completable_assembleNoScope_subscribeInScope() {
    Completable source = scalarCallableCompletable(subscribeContext)
        .doOnComplete(this::assertInSubscribeContext);
    Completable errorSource =
        scalarCallableCompletable(subscribeContext, new IllegalStateException())
            .doOnError(t -> assertInSubscribeContext())
            .doOnComplete(this::assertInSubscribeContext);
  ...
  @Test public void scalarCallable_completable_unwrappedWhenNotInScope() {
    assertThat(scalarCallableCompletable(null)).isInstanceOf(ScalarCallableCompletable.class);
  }

  // ScalarCallable is only returning a constant. Ensure we don't accidentally instrument it.
  @Test public void scalarCallable_completable_uninstrumented() {
    currentTraceContext = throwingCurrentTraceContext;
    setup();

    // currentTraceContext.newScope would throw and break the test if call() was instrumented
    ((ScalarCallable) scalarCallableCompletable(subscribeContext)).call();
  }

  @Test public void scalarCallable_flowable_assembleInScope_subscribeNoScope() {
    Flowable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableFlowable(null)
          .doOnNext(e -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableFlowable(null, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_flowable_assembleInScope_subscribeInScope() {
    Flowable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableFlowable(subscribeContext)
          .doOnNext(e -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableFlowable(subscribeContext, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_flowable_assembleNoScope_subscribeInScope() {
    Flowable<Integer> source = scalarCallableFlowable(subscribeContext)
        .doOnNext(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
    Flowable<Integer> errorSource =
        scalarCallableFlowable(subscribeContext, new IllegalStateException())
            .doOnError(t -> assertInSubscribeContext())
            .doOnComplete(this::assertInSubscribeContext);
  ...
  @Test public void scalarCallable_flowable_unwrappedWhenNotInScope() {
    assertThat(scalarCallableFlowable(null)).isInstanceOf(ScalarCallableFlowable.class);
  }

  @Test public void scalarCallable_flowable_conditional_assembleInScope_subscribeNoScope() {
    Flowable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableFlowable(null)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_flowable_conditional_assembleInScope_subscribeInScope() {
    Flowable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableFlowable(subscribeContext)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_flowable_conditional_assembleNoScope_subscribeInScope() {
    Flowable<Integer> source = scalarCallableFlowable(subscribeContext)
        .filter(lessThanThreeInSubscribeContext)
        .doOnNext(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
  ...
  @Test public void scalarCallable_flowable_conditional_unwrappedWhenNotInScope() {
    assertThat(scalarCallableFlowable(null).filter(i -> i < 3))
        .isInstanceOf(FlowableFilter.class);
  }

  // ScalarCallable is only returning a constant. Ensure we don't accidentally instrument it.
  @Test public void scalarCallable_flowable_uninstrumented() {
    currentTraceContext = throwingCurrentTraceContext;
    setup();

    // currentTraceContext.newScope would throw and break the test if call() was instrumented
    ((ScalarCallable) scalarCallableFlowable(subscribeContext)).call();
  }

  @Test public void scalarCallable_observable_assembleInScope_subscribeNoScope() {
    Observable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableObservable(null)
          .doOnNext(e -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableObservable(null, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_observable_assembleInScope_subscribeInScope() {
    Observable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableObservable(subscribeContext)
          .doOnNext(e -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableObservable(subscribeContext, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_observable_assembleNoScope_subscribeInScope() {
    Observable<Integer> source = scalarCallableObservable(subscribeContext)
        .doOnNext(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
    Observable<Integer> errorSource =
        scalarCallableObservable(subscribeContext, new IllegalStateException())
            .doOnError(t -> assertInSubscribeContext())
            .doOnComplete(this::assertInSubscribeContext);
  ...
  @Test public void scalarCallable_observable_unwrappedWhenNotInScope() {
    assertThat(scalarCallableObservable(null)).isInstanceOf(ScalarCallableObservable.class);
  }

  @Test public void scalarCallable_observable_conditional_assembleInScope_subscribeNoScope() {
    Observable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableObservable(null)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_observable_conditional_assembleInScope_subscribeInScope() {
    Observable<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableObservable(subscribeContext)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_observable_conditional_assembleNoScope_subscribeInScope() {
    Observable<Integer> source = scalarCallableObservable(subscribeContext)
        .filter(lessThanThreeInSubscribeContext)
        .doOnNext(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
  ...
  @Test public void scalarCallable_observable_conditional_unwrappedWhenNotInScope() {
    assertThat(scalarCallableObservable(null).filter(i -> i < 3))
        .isInstanceOf(ObservableFilter.class);
  }

  // ScalarCallable is only returning a constant. Ensure we don't accidentally instrument it.
  @Test public void scalarCallable_observable_uninstrumented() {
    currentTraceContext = throwingCurrentTraceContext;
    setup();

    // currentTraceContext.newScope would throw and break the test if call() was instrumented
    ((ScalarCallable) scalarCallableObservable(subscribeContext)).call();
  }

  @Test public void scalarCallable_single_assembleInScope_subscribeNoScope() {
    Single<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableSingle(null)
          .doOnSuccess(e -> assertInAssemblyContext());
      errorSource = scalarCallableSingle(null, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext());
    }
  ...
  @Test public void scalarCallable_single_assembleInScope_subscribeInScope() {
    Single<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableSingle(subscribeContext)
          .doOnSuccess(e -> assertInAssemblyContext());
      errorSource = scalarCallableSingle(subscribeContext, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext());
    }
  ...
  @Test public void scalarCallable_single_assembleNoScope_subscribeInScope() {
    Single<Integer> source = scalarCallableSingle(subscribeContext)
        .doOnSuccess(e -> assertInSubscribeContext());
    Single<Integer> errorSource =
        scalarCallableSingle(subscribeContext, new IllegalStateException())
            .doOnError(t -> assertInSubscribeContext());

    subscribeInDifferentContext(source.toObservable(), errorSource.toObservable()).assertResult(1);
  }

  @Test public void scalarCallable_single_unwrappedWhenNotInScope() {
    assertThat(scalarCallableSingle(null)).isInstanceOf(ScalarCallableSingle.class);
  }

  @Test public void scalarCallable_single_conditional_assembleInScope_subscribeNoScope() {
    Maybe<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableSingle(null)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_single_conditional_assembleInScope_subscribeInScope() {
    Maybe<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableSingle(subscribeContext)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_single_conditional_assembleNoScope_subscribeInScope() {
    Maybe<Integer> source = scalarCallableSingle(subscribeContext)
        .filter(lessThanThreeInSubscribeContext)
        .doOnSuccess(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
    Maybe<Integer> errorSource = scalarCallableSingle(subscribeContext, new IllegalStateException())
        .filter(lessThanThreeInSubscribeContext)
        .doOnError(t -> assertInSubscribeContext());
  ...
  @Test public void scalarCallable_single_conditional_unwrappedWhenNotInScope() {
    assertThat(scalarCallableSingle(null).filter(i -> i < 3))
        .isInstanceOf(MaybeFilterSingle.class);
  }

  // ScalarCallable is only returning a constant. Ensure we don't accidentally instrument it.
  @Test public void scalarCallable_single_uninstrumented() {
    currentTraceContext = throwingCurrentTraceContext;
    setup();

    // currentTraceContext.newScope would throw and break the test if call() was instrumented
    ((ScalarCallable) scalarCallableSingle(subscribeContext)).call();
  }

  @Test public void scalarCallable_maybe_assembleInScope_subscribeNoScope() {
    Maybe<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableMaybe(null)
          .doOnSuccess(e -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableMaybe(null, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_maybe_assembleInScope_subscribeInScope() {
    Maybe<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableMaybe(subscribeContext)
          .doOnSuccess(e -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
      errorSource = scalarCallableMaybe(subscribeContext, new IllegalStateException())
          .doOnError(t -> assertInAssemblyContext())
          .doOnComplete(this::assertInAssemblyContext);
    }
  ...
  @Test public void scalarCallable_maybe_assembleNoScope_subscribeInScope() {
    Maybe<Integer> source = scalarCallableMaybe(subscribeContext)
        .doOnSuccess(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
    Maybe<Integer> errorSource = scalarCallableMaybe(subscribeContext, new IllegalStateException())
        .doOnError(t -> assertInSubscribeContext())
        .doOnComplete(this::assertInAssemblyContext);
  ...
  @Test public void scalarCallable_maybe_unwrappedWhenNotInScope() {
    assertThat(scalarCallableMaybe(null)).isInstanceOf(ScalarCallableMaybe.class);
  }

  @Test public void scalarCallable_maybe_conditional_assembleInScope_subscribeNoScope() {
    Maybe<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableMaybe(null)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_maybe_conditional_assembleInScope_subscribeInScope() {
    Maybe<Integer> source, errorSource;
    try (Scope scope = currentTraceContext.newScope(assemblyContext)) {
      source = scalarCallableMaybe(subscribeContext)
          .filter(lessThanThreeInAssemblyContext)
  ...
  @Test public void scalarCallable_maybe_conditional_assembleNoScope_subscribeInScope() {
    Maybe<Integer> source = scalarCallableMaybe(subscribeContext)
        .filter(lessThanThreeInSubscribeContext)
        .doOnSuccess(e -> assertInSubscribeContext())
        .doOnComplete(this::assertInSubscribeContext);
  ...
  @Test public void scalarCallable_maybe_conditional_unwrappedWhenNotInScope() {
    assertThat(scalarCallableMaybe(null).filter(i -> i < 3))
        .isInstanceOf(MaybeFilter.class);
  }

  // ScalarCallable is only returning a constant. Ensure we don't accidentally instrument it.
  @Test public void scalarCallable_maybe_uninstrumented() {
    currentTraceContext = throwingCurrentTraceContext;
    setup();

    // currentTraceContext.newScope would throw and break the test if call() was instrumented
    ((ScalarCallable) scalarCallableMaybe(subscribeContext)).call();
  }
