...
public class HttpSpanCollectorTest {
  ...
  @Rule
  public final ZipkinRule server = new ZipkinRule();
  ...
  HttpSpanCollector collector = new HttpSpanCollector(server.httpUrl(), config, metrics);
  ...
  public void collectDoesntDoIO() throws Exception {
    ...
    assertThat(server.httpRequestCount()).isZero();
  }
  ...
  public void dropsWhenQueueIsFull() throws Exception {
    ...
    assertThat(server.collectorMetrics().spans()).isEqualTo(1000);
  ...
  public void postsSpans() throws Exception {
    ...
    assertThat(server.httpRequestCount()).isEqualTo(1);

    // Now, let's read back the spans we sent!
    assertThat(server.getTraces()).containsExactly(
  ...
  public void incrementsDroppedSpansWhenServerErrors() throws Exception {
    server.enqueueFailure(HttpFailure.sendErrorResponse(500, "Server Error!"));
  ...
  public void incrementsDroppedSpansWhenServerDisconnects() throws Exception {
    server.enqueueFailure(HttpFailure.disconnectDuringBody());
