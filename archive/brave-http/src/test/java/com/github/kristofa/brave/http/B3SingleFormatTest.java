...
import static com.github.kristofa.brave.http.B3SingleFormat.parseB3SingleFormat;
...
public class B3SingleFormatTest {
  ...
  @Test public void parseB3SingleFormat_idsNotYetSampled() {
    assertThat(parseB3SingleFormat(traceId + "-" + spanId).getSpanId())
        .isEqualToComparingFieldByField(
            SpanId.builder().traceId(1).spanId(3).parentId(null).build()
        );
  }

  @Test public void parseB3SingleFormat_idsNotYetSampled128() {
    assertThat(parseB3SingleFormat(traceId + traceId + "-" + spanId).getSpanId())
        .isEqualToComparingFieldByField(
            SpanId.builder().traceIdHigh(1).traceId(1).parentId(null).spanId(3).build()
        );
  }

  @Test public void parseB3SingleFormat_idsUnsampled() {
    assertThat(parseB3SingleFormat(traceId + "-" + spanId + "-0").getSpanId())
        .isEqualToComparingFieldByField(
            SpanId.builder().traceId(1).spanId(3).parentId(null).sampled(false).build()
        );
  }

  @Test public void parseB3SingleFormat_parent_unsampled() {
    assertThat(parseB3SingleFormat(traceId + "-" + spanId + "-0-" + parentId).getSpanId())
        .isEqualToComparingFieldByField(
            SpanId.builder().traceId(1).parentId(2L).spanId(3).sampled(false).build()
        );
  }

  @Test public void parseB3SingleFormat_parent_debug() {
    assertThat(parseB3SingleFormat(traceId + "-" + spanId + "-d-" + parentId).getSpanId())
        .isEqualToComparingFieldByField(
            SpanId.builder().traceId(1).parentId(2L).spanId(3).sampled(true).debug(true).build()
        );
  }

  @Test public void parseB3SingleFormat_idsWithDebug() {
    assertThat(parseB3SingleFormat(traceId + "-" + spanId + "-d").getSpanId())
        .isEqualToComparingFieldByField(
            SpanId.builder().traceId(1).spanId(3).parentId(null).sampled(true).debug(true).build()
        );
  }

  @Test public void parseB3SingleFormat_sampledFalse() {
    assertThat(parseB3SingleFormat("0").getSample())
        .isFalse();
  }

  @Test public void parseB3SingleFormat_sampled() {
    assertThat(parseB3SingleFormat("1").getSample())
        .isTrue();
  }

  @Test public void parseB3SingleFormat_debug() {
    assertThat(parseB3SingleFormat("d").getSample())
        .isTrue();
  }
